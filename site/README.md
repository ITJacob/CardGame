...
- **卡片浏览** `#/cards`：按途径/构筑轴/稀有度/类型/序列/原语/距离筛选 + 关键词搜索 + 按稀有度排序，卡片详情含效果 AST 的中文精读
  ...
  - **轴符号的提亮**（`js/axis-ink.js`）：水印是低透明度的 emoji（列表卡 `opacity:.16`、详情页 `.18`），本身近黑的符号（🕳️ 污秽 / 🐾 兽群 / 🎩 戏法 / 🌑 暗夜…）铺在近黑卡面上等于一块暗斑，看不见。所以渲染前用一块 64×64 canvas 把该符号画一遍、按 WCAG 相对亮度算不透明像素的平均明度，低于 `0.15` 的才加 `.ax-lift`（全库 88 个符号命中 12 个，实测值 0.041 → 0.727 是连续分布，这条线是人为定的）。**不是**全量套滤镜：早先一刀切 `brightness(0) invert(1)` 会把每个 emoji 按 alpha 压成单色剪影，🕸️ 的网眼、⛓️ 的链环全塌成实心块、颜色全丢——修好 12 个弄坏 88 个。改成 `invert(1) hue-rotate(180deg)`：只翻明暗、大致保住色相与字形内部的相对明暗（恶魔提亮后仍是红的），两件事必须一起做，只 `invert` 会把暗红翻成亮青。**现算而不是写死一张深色清单**：符号换过一批（2026-09），且同一码位在 Segoe UI Emoji / Apple Color Emoji / Noto Color Emoji 下深浅不同——清单只能代表写它的那台机器，现算量的是读者这台机器上正在渲染的那版字形。测不出来（canvas 被策略禁用、字体缺失）一律不改动，退回 emoji 原样
    - **同一份判定还管着 badge 与悬停浮层**：轴符号不只铺在卡面水印上——列表卡与详情页的轴 badge（`--panel2` 底）、悬停浮层的轴标题，底色一样是近黑，那 12 个深色符号在那里同样是块看不见的暗斑。所以 `lookupAxis` 把 `symbol` / `name` 拆开返回，三处共用 `axSymHtml()` 现算并打 `.ax-lift`（实测全库 88 个轴符号，三处命中同一批 12 个）。**符号必须单独包一层 `<span class="ax-sym">`**：滤镜套在整段文字上，紧挨着的中文轴名会跟着翻成反色。筛选下拉的原生 `<option>` 不在此列——交给系统渲染，滤镜够不着
  ...
  - **详情页 = 整页一张卡**（`body.page-card` + `.card-stage/.card-sheet`）：顶栏隐藏，页面本身作画布——`::before` 铺一层 `position:fixed` 的途径色氛围背景（不用 `background-attachment:fixed`，iOS 上失效），内容收进居中的大卡。稀有度在这张卡上决定框体规格：`--rc` 顶部色线 + 边框着色递进（uncommon 起有色边框，epic 加内描边，legendary 金框 + 斜纹底纹 + 外发光）；途径色做卡面顶部晕染，轴符号放大为右上角水印（深色符号的提亮同上，`.card-sheet.ax-lift`）。小节用发丝分隔线 + 小字标题，不再嵌套面板
