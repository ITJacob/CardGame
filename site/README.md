...
- **卡片浏览** `#/cards`：按途径/构筑轴/稀有度/类型/序列/原语/距离筛选 + 关键词搜索，卡片详情含效果 AST 的中文精读
  - **卡片视觉编码**（纯 CSS，无动画无滤镜，移动端零额外重绘）：稀有度五档递进——common 朴素 / uncommon 左色条 / rare 着色边框 / epic 边框+顶部高光线 / legendary 金框+斜纹 SVG 底纹；途径特色 = 22 色手选调色板（`--p-*`，原著气质），落在卡片背景的 8% 晕染、途径 badge 着色与右下角轴符号水印（`data-ax` 伪元素，`attr()` 取字符无额外 DOM）。`color-mix` 不支持的旧浏览器整条背景声明失效、退回纯色底，属渐进增强
