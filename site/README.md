...
- **卡片浏览** `#/cards`：按途径/构筑轴/稀有度/类型/序列/原语/距离筛选 + 关键词搜索 + 按稀有度排序，卡片详情含效果 AST 的中文精读
  ...
  - **AI 出图小节**（有 `art` 的卡才显示）：画面与边框是**两张独立的图**。画面 prompt = 全局画幅（`manifest.artFormat`，3:4 竖版无边框无文字）+ 途径顶层 `artStyle` + 卡级 `art` 三件套（subject / elements / mood）；边框 prompt = 全局边框格式（`manifest.artFrameFormat`，3:4 竖版 + 手绘媒介 + 纯黑底 + 中心留空 + 四边等宽 + 无文字水印）+ 各档材质层——common/uncommon/rare 走 `manifest.artFrame` 全局三档（中性配色，不抢途径色），epic/legendary 走途径顶层 `artFrameEpic`/`artFrameLegendary`（结合途径元素特殊化）。全部展示时现拼（`card.js` 的 `artPrompt`/`frameFor`），不落盘——换风格只改途径一处，换画幅/换边框格式只改 manifest 一处。两个复制按钮分开复制。成品图：画面存 `site/assets/cards/<cardId>.webp`，边框存 `site/assets/frames/`（三档 `frame-<rarity>.webp`，两档 `frame-<途径>-<档>.webp`），详情页按约定路径现拼、`<img onerror>` 移除——存在即显示，无需索引文件；边框图黑底 + CSS `mix-blend-mode: screen` 叠在画面上。重出图同名覆盖
