...
- **卡片浏览** `#/cards`：按途径/构筑轴/稀有度/类型/序列/原语/距离筛选 + 关键词搜索 + 按稀有度排序，卡片详情含效果 AST 的中文精读
  ...
  - **AI 出图小节**（有 `art` 的卡才显示）：完整 prompt = 途径顶层 `artStyle` + 卡级 `art` 三件套（subject / elements / mood），展示时现拼（`card.js` 的 `artPrompt`），不落盘——换风格只改途径文件一处，不动 829 张卡。附一键复制按钮（站点首个剪贴板交互，`navigator.clipboard` + 失败退回全选文本）。成品图存 `site/assets/cards/<cardId>.webp`，详情页按 id 现拼路径、`<img onerror>` 移除——存在即显示，无需索引文件；重出图同名覆盖
