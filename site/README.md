...
- 条目**只有两个来源**（`js/md.js` 的 `parseRef`，规则写在那里）：**表格行**与**带标识符的小节标题**。
  - 收：首列是「名字」的字段表（`字段 / 谓词 / 编号 / 维度 / 母版 …`，清单是 `NAME_COLS`，加新的表就把列首加到那里）。表头原样当条目的字段标签，文档写明的语义不在前端重新解释。
  - 不收：首列是序号或纯数值的**数据表**（标定梯度、时相表、分布表、占位盘点）、标题是纯中文的**散文小节**（`职责定位`）、以及只剩标题而正文全在表格里的空壳概念。它们留给每篇末尾的「原文」。
