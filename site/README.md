...
每次重开都要重下那 450 KB。SW 用 stale-while-revalidate 把 `docs/` 下的数据文件缓存起来，
命中后重开只需约 1.2 秒（剩下的全是 app shell），数据传输 0 字节。

- **只缓存数据，不缓存 app shell**：数据和代码的失效节奏不同，shell 一旦也吃缓存，部署后
  就会出现「旧 JS 配新数据」这种最难查的组合。数据是只读内容，先给缓存里的那份、后台静默
  换新，代价是数据更新后最多有一次加载看到上一版。
- **索引（`manifest.json`）例外，走网络优先**：它才 1 KB，且决定「有哪些途径」。吃到旧的
  索引会让新加的途径整条不出现——连请求都不会发，比某张卡晚一版更容易让人以为丢了数据。
