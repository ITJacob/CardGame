...
- **页脚「抽取情况」把漏抽变成可见的**：`parseRef` 每跳掉一张表就记一条（行号 + 小节 + 表头），聚合成 `<details class="panel qr-coverage">` 挂在正文末尾。抽取规则是前端硬编码的，文档那边不可能知道——新写一张字段表而首列名不在 `NAME_COLS` 里，它不报错、页面上也没痕迹，只是从速查里消失。列出跳过的表之后，跳的是数值表还是漏掉的字段表，扫一眼就分得出来。
