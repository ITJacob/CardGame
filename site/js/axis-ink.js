// 轴符号水印的可见性判定：这个 emoji 铺在近黑卡面上够不够亮？
//
// 为什么要有这个模块：水印是 opacity .08 的 emoji，本身不发光的深色 emoji（🕳️ 污秽
// 0.04、🐾 兽群 0.05、🎩 戏法 0.07、🌑 暗夜 0.12）铺在近黑卡面（#1d2026，相对亮度
// ≈0.014）上就是一块暗斑，读者看不见——这正是「不眠者的暗夜轴图标不出现在背景里」
// 的成因。早先的修法是一刀切给**全部** 88 个符号加 brightness(0) invert(1)，代价是
// 把每个 emoji 都拍成单色剪影：🕸️ 的网眼、⛓️ 的链环、🎩 的帽带全塌成实心块，
// 颜色也全丢——修好了 12 个，弄坏了 88 个。
//
