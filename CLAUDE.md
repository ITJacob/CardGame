...
6. **改 JSON 结构先改 schema**：`docs/json/schema/skills.schema.json` 是技能池的结构标准（规范说明在 `docs/meta/SCHEMA.md`）。新增/删除字段、扩枚举都**先改 schema 再改数据**；改完跑 `docs/tools/validate_schema.py`（结构）+ `docs/tools/validate.py`（语义），两者都要 0 错误。
