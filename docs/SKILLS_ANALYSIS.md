...
> 数据来源：`docs/json/*.skills.json`（22 份途径技能池，共 **817 张卡**），生成脚本口径见文末。
...
**卡数分布**：律师 46（最多）；战士/秘祈人 33（最少）；均值约 37（817/22）。单途径规模因 #4/#6 补卡整体抬升（见 §七·补 7-4 / 7-6）。
...
- 数据源：`docs/json/*.skills.json` 的 `cards[]` 数组，共 817 张卡。
