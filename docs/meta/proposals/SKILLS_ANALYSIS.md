...
> 数据来源：`docs/json/*.skills.json`（22 份途径技能池，共 **828 张卡**），生成脚本口径见文末。
...
|---|---|---|---|
| 途径数 | 22 | 22 | ✅ |
| 每途径卡数 | 约 24 | **33–46**（均值约 37） | ⚠️ 超出约 55%，规模偏大 |
| 序列覆盖 | 覆盖 10 个序列 | 22/22 途径均 **10/10** | ✅ 优秀 |
| 构筑轴 | 每途径 4 轴 | 22/22 途径均 4 轴 | ✅ |
| 轴间均衡 | 「4 轴尽量均衡」 | ⚠️ 仍不均，但全部弱轴已补至 10（22 途径 max/min 已收敛到 **1.6–2.3x**） | 🟢 基本收敛（见 §5.2 / 问题 #4 #6） |
| 旗舰技能 | 每轴 1 个（= 4） | **22/22 途径恰好 4 个（全库 88）** | ✅ 已收敛（见 §七·补 7-7） |
| 主动 / 被动 | 4+4（派发槽位，非池内比例） | 池内约 6:4 | ⚠️ 被动占比偏高 |

**卡数分布**：律师 46（最多）；战士/秘祈人 33（最少）；均值约 38（828/22）。单途径规模因 #4/#6/#7 补卡整体抬升（见 §七·补 7-4 / 7-6 / 7-7）。
...
| **律师** | advocacy(10) / brute(10) / bribery(10) / **warp_rule(16)** | 辩护·暴力·贿赂·**扭曲规则** | ✅ 已收口（disorder→warp_rule 改名 + 语义重分类 + 补弱轴，max/min 1.6x） |
...
1. ✅ **律师途径已收口**（2026-09-16，详见 §七·补 7-6）：`disorder` 改名 `warp_rule`（扭曲规则，贴合黑皇帝"改写规则"权柄）；随后做**语义重分类**——把明显不属于"规则扭曲"的两张卡移出（`s2_moon_step` 踏月＝强化物理攻击、`s3_frantic_adaptation` 狂乱适应＝越挨打越强，二者更属 `brute`），`warp_rule` 18→16；再把 `brute` 4→10、`bribery` 7→10。最终 **advocacy 10 / brute 10 / bribery 10 / warp_rule 16，max/min = 1.6x**，与其他途径同档。
2. ⚠️ **轴间失衡已基本收敛**：全库弱轴均达 10 张下限，22 途径 max/min 统一落在 **1.6–2.3x**（apothecary 1.7x、chanter 2.3x、corpse_collector 2.3x、hunter 2.0x、sailor 1.8x、lawyer 1.6x）。策略始终是**"补弱轴到下限"而非"缩强轴"**——因为强轴往往是该途径的签名权柄（如 chanter 的 light、hunter 的 massing、lawyer 的 warp_rule），强行削减会伤害主题浓度。残留的 1.6–2.3x 差值是刻意保留的主题权重，不再视为缺陷。
...
>
> 吟唱的问题本质：v0.3 规定「序列 4 以上或仪式 / 咒文类必须标具体 tick 数」，而 526 张带该字段的卡里 257 张显式 `0`、232 张 `null`——**这条纪律几乎从未执行**。现已补档位规则（`GENERATION_BRIEF.md` §13.2）并回填 38 张。

### 6.2 严重低用 ⚠️

| 工具 | 用量 | 应然 |
|---|---|---|
| 位格 `rank` | ~~7 卡 / 4 途径~~ → **10 卡 / 4 途径** | v0.3 头号核心数值；已指定**仲裁人**做示范（+4，见 7-7） |
| 跨层 `translocate` | ~~0 卡~~（原记 2 卡实为文案提及）→ **4 卡 / 1 途径** | **根因是 schema 缺该原语**（ddd 11 号，2026-09-06 拍板但 schema 未登记）→ 已补登记 + **愚者**示范（+4） |
| 形态切换 `formGroup` | ~~2 卡 / 1 途径~~ → **5 卡 / 2 途径** | 魔女/野兽变形系的天然载体；已指定**刺客**做示范（+3 形态轮转） |
...
| 4 | 轴间失衡 3–5 倍 | 🟢 已收敛（1.6–2.3x） | 2026-09-16 已为 6 弱轴各补 6–7 张新卡（共 +37）：rage / hymn / intrigue / beasts / spirit_sight / advocacy 均达 10（清单见 §七·补 7-4）；律师另经 #6 收口至 1.6x。全库 22 途径 max/min 现为 1.6–2.3x |
| 5 | 旗舰标记 4~16 失控 | ✅ 已解决 | 2026-09-16 已收敛：**237 → 88**，22/22 途径恰好每轴 1 个（规则：每轴恰好 1 张者原样保留；多者保留序列位阶最高者）。详见 §七·补 7-7 |
| 6 | 律师 `warp_rule`（原 `disorder`）卡量 18 偏大、与原著权柄曾偏离 | ✅ 已解决 | 2026-09-16：改名 warp_rule「扭曲规则」→ 语义重分类移出 2 张（踏月/狂乱适应→brute，warp_rule 18→16）→ 补 brute 4→10、bribery 7→10。最终 10/10/10/16，max/min **1.6x**（详见 §七·补 7-6） |
| 7 | 位格 / 跨层 / 形态切换低用 | ✅ 已解决 | 2026-09-16 指定三条途径做深度示范：**仲裁人=位格压制(+4)、愚者=跨层(+4)、刺客=形态轮转(+3)**。关键发现：`translocate` 零使用是**因 schema 缺该原语**（ddd 已拍板未同步），已补登记。详见 §七·补 7-7 |
...
按序列档位（序列 4→2t、3→3t、2→4t、1→5t、0→6t）赋给「序列 ≤ 4 且主动且界域 / 仪式类」的卡；档位规则已写入 `GENERATION_BRIEF.md` §13.2。**数值补偿（吟唱 N tick ≈ 折扣 0.5×N 能量）尚未逐卡回填，落编目时需补。**
...
| 律师 lawyer | advocacy 4→**10** | +6 | 4.5x（warp_rule 18）→ 同日经 #6 收口至 **1.6x**，见 7-6 |
...
**结果**：整库卡数 773 → **810**；6 弱轴均达 10 张，途径内 max/min 由 3–5x 降至 1.7–2.3x（律师因 warp_rule 仍 18、brute 仅 4，保持 4.5x，随后由 #6 收口至 1.6x，见 7-6）。校验：`validate.py` 0 error / 19 warn；`validate_schema.py` 当时仅余 3 个历史错误（assassin s18、fool s28、prisoner s21），manifest 计数已全部同步——**该 3 项已于同日另行清零，见 7-5**。
...
### 7-6 律师途径收口：语义重分类 + 补弱轴（2026-09-16，问题 #6 解决）

**背景**：#4 补弱轴后律师仍为全库唯一 4.5x 失衡（warp_rule 18 / advocacy 10 / bribery 7 / brute 4）。裁定采用「语义重分类 + 少量补卡」——先修正归类错误，再把弱轴补到 10，兼顾语义准确与均衡，且不至于让单途径卡数失控。

**第一步·语义重分类**（把明显不属于「扭曲规则」的卡移出 `warp_rule`）：

| 卡 | 原名 | 为何不属于 warp_rule | 移入 |
|---|---|---|---|
| `skill_lawyer_s2_moon_step` | 踏月 | 强化下次攻击 ×2.5、无视站位与护甲——是纯物理强攻，与"改写规则"无关 | `brute` |
| `skill_lawyer_s3_frantic_adaptation` | 狂乱适应 | 每持有 1 个减益攻击 +1——是"越挨打越强"的狂战士逻辑，属蛮力 | `brute` |

`warp_rule` 18 → **16**；`brute` 4 → 6。

**第二步·补弱轴至 10**（+7 张）：

- **brute +4**：`s6_seize` 强夺（伤害 5 + 自身攻击 +3）、`s5_violent_verdict` 暴力裁决（伤害 6，目标 HP<50% 追加 6）、`s2_crush` 碾压（伤害 8、无视护甲、溅射相邻）、`s1_regicide_fist` 弑序铁拳（伤害 14、无视护甲）
- **bribery +3**：`s5_buy_off` 收买（被收买 4t：造成伤害 ×0.8、受治疗 ×0.5）、`s3_heavy_bribe` 重贿（腐蚀 3t 溅射相邻，每 tick 2 物理伤害）、`s1_universal_bribe` 万贿归一（敌方全体被收买 5t：造成伤害 ×0.85）

**结果**：律师 39 → **46** 张；四轴 **advocacy 10 / brute 10 / bribery 10 / warp_rule 16**，max/min 由 **4.5x 降至 1.6x**，与其他途径（1.7–2.3x）同档。整库 810 → **817**。

**为何保留 warp_rule 为最大轴**：改名后「扭曲规则」正是黑皇帝的核心权柄（改写规则、定义、重定义），作为签名轴略大于其余三轴是**主题正确**的，不再视为缺陷；#6 真正要修的是"归类错误 + brute/bribery 过小"，已全部处理。

**校验**：`validate.py` 0 error / 19 warn；`validate_schema.py` 0 error / 28 warn；manifest 已同步（lawyer 46、totalCards 817）；`SKILLS_OVERVIEW.md` 已重跑 `build_overview.py` 重新生成。

### 7-7 旗舰收敛 + 位格/跨层/形态示范（2026-09-16，问题 #5 #7）

#### 一、旗舰标记收敛：237 → 88（#5）

**规则**（确定性、可复现、且不动已正确的途径）：

1. 每个 (途径, 轴) 恰好 1 张 flagship → **原样保留**（尊重作者既有判断）
2. 每个 (途径, 轴) 多于 1 张 → 保留其中**序列位阶最高（`sequence` 最小）**的那张；同序列按 `id` 字典序破平
3. 全库**每个轴都已有 ≥1 张**，故只裁剪、不新增——作者原始意图零丢失

**为何不能纯按"最低序列"一刀切**：先核对了已正确的三个样板（sailor / savant / sleepless 各恰好 4 张），发现作者选的**并非**最低序列——sailor 的 rage 轴最低是 s2，旗舰却是 s8 暴怒一击；savant 的 lore 轴最低 s0，旗舰却是 s9 唤醒记忆。可见旗舰的语义是「最具代表性」而非「位阶最高」。因此规则 1 优先保留既有选择，只在轴内溢出时才用位阶破平。

**结果**：移除 149 个 flagship 标记，**22/22 途径均恰好 4 张，全库 88**。保留结果多为各途径顶点权柄卡（律师=独裁者、水手=暴君权柄、收尸人=死亡权柄、阅读者=白塔·全知、耕种者=万物母巢…）。三个样板途径**完全未被改动**。

#### 二、位格 / 跨层 / 形态切换示范（#7）

**关键发现：`translocate` 零使用的根因不是"没人写"，而是 schema 缺这个原语。** 它是 ddd 效果参数 11 号（2026-09-06 拍板，`op: pull_into|banish`），但 `skills.schema.json` 里根本没有对应分支——与 `atk_asc` 同属「ddd 已登记、schema 未同步」。已补 `effTranslocate`（required `type/op/duration`，含 `returnPayload`），原语总数 18 → **19**，`SCHEMA.md` §5.2 同步登记。

| 机制 | 途径 | 新增卡 | 说明 |
|---|---|---|---|
| **位格 `rank`** | 仲裁人 | 4 | s5 位格管辖（位格低者打你 ×0.9，常驻 rank 双乘区）／s2 位格制裁（位格高则 9 伤+眩晕 2t，否则 4 伤）／s1 贬谪（`modify_stat rank -1` 4t + 受伤 +20%）／s0 位格律令（对位格低者伤害 ×1.25）。比较一律走 `stat_compare(key:'rank')`，遵守"位格比较不新增谓词" |
| **跨层 `translocate`** | 愚者 | 4 | s5 灵界放逐（banish 敌 3t）／s2 灵界庇护（pull_into 友 2t + 回归治疗 6）／s3 遁入灵界（banish 自身 1t 规避）／s1 秘偶置换（banish 敌 2t + 回归 5 精神伤害）。**均补了 `modify_resource(lost)` 迷失租金** |
| **形态 `formGroup`** | 刺客 | 3 | 影（伤害 ×1.15）/ 雾（受伤 ×0.85）/ 血（吸血 +2）三形态，同属 `assassin_form` 组，经 `nextPhase` + `cycleTicks:5` 互斥轮转（H50） |

**踩坑记录**：`validate.py:123` 有硬规则 `domain/translocate 必须付迷失租金（resource:"lost"）`——首轮 4 张愚者卡全被拦下，补齐后才过。`lost` 是独立资源槽（非 hp/energy），全库已有 27 处使用，先例见 apothecary s0 血月（界域租金）。**写跨层/界域类卡时一并加租金，别等校验报错。**

**结果**：位格 6→**10 卡 / 4 途径**；跨层 0→**4 卡**；形态 2→**5 卡 / 2 途径**。整库 817 → **828**。

**校验**：`validate.py` 0 error / 19 warn；`validate_schema.py` 0 error / 28 warn；manifest 同步（arbiter 40 / fool 38 / assassin 39、totalCards 828）；`SKILLS_OVERVIEW.md` 已重跑生成。
...
2. **原著契合**：22/22 途径的构筑轴均能在原著找到明确对应；律师 `disorder` 已于 2026-09-16 改名 `warp_rule`（扭曲规则）以贴合黑皇帝"改写规则"权柄，并经语义重分类 + 补弱轴收口至 max/min 1.6x（#6 已解决）。
3. **工具覆盖**：主流工具（界域、迷失、充能、推条、标签联动、叠层、转嫁、潜行、复活）利用率良好；v0.3 三大新增维度中**吟唱已修复（1 张 → 38 张）**、**位格（→10 卡 / 4 途径）、跨层（0 → 4 卡）、形态（→5 卡 / 2 途径）已于 2026-09-16 指定途径做深度示范**（见 §七·补 7-7）。其中跨层此前的"零使用"实为 **schema 缺原语**所致，已补登记——**这类"ddd 已拍板、schema 未同步"的缺口应先排查再下"没人用"的结论**。
4. **三件事的处置（2026-09-15 → 2026-09-16）**：`dark` 存量迁移 ✅ 9 张；吟唱回填 ✅ 38 张（档位规则已入 `GENERATION_BRIEF.md` §13.2，数值补偿待回填）；`on_turn_start` 🟡 **方案已定——引擎补 fire 点规格见 §七·补 7-3（含 25 处精确清单：4 hook + 21 event / 19 张卡），待 cardgame-core 落地后关闭 G15**。

---

### 附：统计口径

- 数据源：`docs/json/*.skills.json` 的 `cards[]` 数组，共 828 张卡。
