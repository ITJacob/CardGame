...
| `artFrameEpic` / `artFrameLegendary` | string | – | 途径 epic / legendary 档**边框图**的材质层（英文，结合途径元素——**只写材质与纹样，且一律用颜料语汇**，禁用 brushed / polished / gilded 等金属加工词）。框宽等几何由站点 CSS 决定，不进 prompt。边框与画面是两张图，站点叠合；共用格式约束（`artFrameFormat`）与 common/uncommon/rare 三档材质层（`artFrame`）在 `manifest.json` 全局登记 |
| `cards` | array | ✓ | 技能卡列表 |
