...
| N4 | axes 元数据**跨轴引用** | ~~补轴时发现已填元数据引用了他轴卡……后两处未改值，待统一评审后决定是否一并按同法修正~~ → ✅ **已闭环**（2026-09-26 主人裁定）：corpse_collector/end enablers 改列本轴卡【一切的归宿】（原冥界放出/冥界之门留 undead 轴）；sailor/storm payoffs 换本轴卡【风刃军团/风暴地狱】（原海啸留 ocean 轴）。与 spectator/fantasy 同法修正。 | ✅ **已闭环** |
...
- ~~**复活优先级链未定（重要）**~~ → ✅ **已拍板**（2026-09-26）：`revive_blocked` > `defined_substitute`（压制其余复活）> `lethal_protect` > `on_death` 复活。已落 ddd 编队参数 §1.1b revive_blocked 行。【九座陵寝】【定义·替身】强度可标定。
- ~~**【狂乱】方差**~~ → ✅ **已拍板**（2026-09-26）：保留真随机（8 段独立伯努利 p=0.25，全稿唯一概率来源），退化方案不启用；已入 ddd 随机性治理 §四登记册。
...
- ~~**未决优先级**：【替罪】`lethal_redirect`→【血肉不灭】`lethal_protect`→【灵肉之刃】`revive_blocked` 三者结算优先级需先定~~ → ✅ 并入复活链统一拍板（2026-09-26，见 编队参数 §1.1b）：revive_blocked 最高，lethal_protect 在 defined_substitute 之下、on_death 之上。
