...
| **性别（Gender）** | 无 | 新增。角色属性 + 可转换能力，见 §12 |
...
性别是**角色属性**，同时「性别转换」本身做成**刺客途径序列 4 的能力**（对应原著）。

| 字段 | 取值 | 说明 |
|---|---|---|
| `gender` | `male` / `female` | 战外确定，作为 `CombatSetup` 初始参数传入；战斗内可被能力改变 |

### 12.2 性别对技能的影响（三条用法，按优先级）

1. **可得性约束**（最常见）：部分技能标注 `**性别** 女`，男性角色无法获得/施放。
   - 典型：刺客途径序列 4 及以上的高位技能限女性。
2. **效果差异**：同一技能，不同性别效果不同（如男性施放衰减）。
   - 只在原著有明确依据时使用，不要为了用性别而用。
3. **转换能力**：把"性别转换"做成主动技能，转换后解锁受性别约束的技能。
   - 典型：**刺客途径序列 7「女巫」**——男性角色施放【女巫之形】后 `gender` 转为 `female`，并解锁后续高位技能（序列 6 欢愉"更加美丽"、序列 0 女性权柄同属此谱系）。这是一个**构筑拐点**，很有戏剧性。
   - ⚠️ 序列以各途径稿的「原著」行为准，**不要照抄"序列 4"**——刺客途径是序列 7。

### 12.3 写法纪律

- **默认不限**：本途径若无原著性别依据，技能条目**不写**性别字段（不要为凑字段硬写）。
- **只在有必要约束时标**：绝大多数技能是"- **性别** 不限"或干脆省略。
- **转换技能要写清转换后解锁了什么**，否则玩家不知道为什么要转。
...
| 7 | **性别 `gender`** 可选属性 + 战斗内转换 | 编队参数 §1.4 + 编队上下文 Unit.gender；转换走扩展算子 `gender_shift`（效果上下文 §2.1）；条件面走 `gender_is` 谓词 | ✅ 已落 |
