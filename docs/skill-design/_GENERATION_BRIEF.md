...
> ⚠️ v0.3 引入的位格 / 界域 / 迷失值 / 性别 / 吟唱字段，**在 `docs/ddd` 中尚无对应定义**。本 brief 先按内容侧约定写死口径，待本轮技能稿改完后统一融入 DDD（清单见 §14）。
...
| 范式 | 定义 | 原著出处 | 落到九原语 |
|---|---|---|---|
| ① **降临** | 让场景覆盖当前区域 | 旅法师序列2、神之画卷 | 切换上层界域（**需新机制**，见 §14） |
| ② **拉入** | 把目标拖进另一个世界 | 不老魔女序列3、织梦人序列3、梦魇序列7 | 目标离场 N 回合（**需新机制**） |
| ③ **放逐** | 丢进随机时空乱流，一段时间后回归 | 秘法师序列4（原著 20 秒） | 拉入 + 回归时随机事件（**需新机制**） |
| ④ **置换** | 当前区域与另一空间对换 | 旧日之盒第一层、末日魔女序列1 | 上下层界域对换（**需新机制**） |
| ⑤ **投影** | 分身/投影降临 | 绝望魔女序列4 | `spawn` **已有**，加"力量封顶"参数 |
| ⑥ **潜入** | 自己躲进另一个世界规避打击 | 秘法师序列4「幻化」、怨魂序列5 | 不可选中 = C8 候选池过滤 **已有**；免疫首次攻击 = `damage_intercept` 母版 |

> ①②⑥ 三种已有 DDD 原语基础，③④⑤ 需要新增能力位。**新增清单见 §14**，本轮技能稿先按口径写，DDD 融入时再落地。
...
- 吟唱期间技能处于"待发"状态，**可被控制效果打断**（眩晕/沉默/冰冻等），打断则技能失效并退还部分能量（⚠️D 退多少待定）。
...
> 本轮技能稿先按上述口径写死。**改完后统一融入 DDD** —— 下面是要新增/修改的位置，按上下文归类，方便直接落地。

| # | 需求 | 建议落点 | 状态 |
|---|---|---|---|
| 1 | **位格 `rank`** 数值（含序列基准表、途径/遗物修正位） | 编队上下文（Unit 属性）+ 编队参数 §一 | 🆕 新增 |
| 2 | **界域 `Domain`** 实体（上层/底层/现实基底三层；环境规则 / 交互规则 / 持续 三字段） | 战场上下文（与 Zone 并列，但作用域为全场） | 🆕 新增 |
| 3 | **上层界域唯一 + 后打出覆盖** 的生命周期规则 | 战场上下文不变量（新增 INV-B9–B10 段） | 🆕 新增 |
| 4 | **拉入 / 放逐 / 置换** 三种跨层操作的效果语义 | 效果上下文（可能是第 10 个效果原语，或 `move` 的扩展 op） | 🆕 新增，需拍板：是新原语还是 move 扩展 |
| 5 | **离场（banish）** 状态：单位离场 N 回合，不触发亡语、不触发反击 | 编队上下文 §十 死亡结算链 + 编队参数 §1.1b | 🆕 新增 |
| 6 | **迷失值 `lost`** 战斗内累积资源 + 锚 `anchor`（来自战外遗物） | 战场上下文（战斗级资源）；`anchor` 作为 CombatSetup 初始参数 | 🆕 新增 |
| 7 | **性别 `gender`** 可选属性 + 战斗内转换 | 编队上下文 Unit；转换走 `modify_stat` 还是新算子待定 | 🆕 新增 |
| 8 | **吟唱 `cast_time`** 正式纳入技能定义（当前只在 BehaviorModifier 里作为状态效果存在） | 执行上下文（技能提交流程）+ 执行参数 §一 | ♻️ 扩展现有 `cast_time_set` |
| 9 | **打断吟唱** 的语义（控制状态打断待发技能） | 执行上下文 + 编队参数（控制状态新增 `interrupt` 标志） | 🆕 新增 |
| 10 | **卡标签 / 界域交互** 的取值域 | 共享内核参数（新增标签枚举）+ 编目参数（SkillDef 元标记） | ♻️ 扩展 |

**待拍板的硬问题**（影响 DDD 结构，本轮先绕开）：
- 第 4 项：拉入/放逐/置换 是**新增第 10 个效果原语**，还是 **`move` 原语的扩展 op**？前者干净但要动九原语的封闭性；后者省事但 `move` 语义会被撑大。
- 第 2 项：界域是**独立聚合实体**，还是**挂在战场上的特殊状态**？建议前者（有独立生命周期与覆盖规则）。
